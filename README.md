...
- Active-route detection distinguishes Wi-Fi from Ethernet when both adapters are connected. Wired connections report their network profile, link state, and negotiated link speed instead of a false 0% radio signal. Internet reachability uses a live TCP connection check and the monitor records adapter-type changes in real time.
...
- Remote Systems includes an AnyDesk launcher. It accepts a remote AnyDesk ID or alias, detects the signed standard executable in common install locations or Downloads, and can select a valid signed AnyDesk executable elsewhere. It starts AnyDesk directly, so a registered Windows URL handler is not required. The viewer, approval, and control remain in AnyDesk. Cybeck does not install, bundle, license, or store credentials for AnyDesk.

- Operations can close the entire local AnyDesk app after confirmation. This also closes unrelated AnyDesk sessions on that PC.
- Editable Tasks link to Network Monitor and Operations, with shortcuts from activity and incident cards. Notes have a lined editor and rotating urgency categories. Both save encrypted locally, with readable JSON export and import for manual backup and future cloud-drive integration.
...
The v0.1.17 GitHub installer is unsigned. Windows may show a SmartScreen or Smart App Control warning, and managed device policy may block it. Do not disable Windows security features or add blanket antivirus exclusions. A trusted publisher signature requires a valid code-signing certificate or signing service; a self-signed certificate does not establish public trust.

To resolve the Smart App Control block, obtain a publicly trusted RSA code-signing identity or configure Microsoft's Artifact Signing public-trust service. For a certificate file, supply electron-builder with `WIN_CSC_LINK` and `WIN_CSC_KEY_PASSWORD` through your local environment or protected CI secrets. Keep the private key and password out of the repository. Run `npm run build:release` and `npm run verify:release` for a trusted installer; the verifier checks the installer, app executable, and elevation helper. The `npm run publish` command also requires code signing. The v0.1.17 build remains unsigned because a signing identity is not available. Create a new version and test its signed installer on a Windows device with Smart App Control enabled. A valid signature does not guarantee immediate SmartScreen reputation or acceptance under every managed-device policy.

## Local Test Mode

Run `Start Cybeck Test Mode.cmd` from the project folder to test the current working build without invoking the packaged updater. This testing path does not change Windows security settings and does not make an unsigned installer trusted.
